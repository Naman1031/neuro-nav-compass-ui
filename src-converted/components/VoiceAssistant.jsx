...
export const VoiceAssistant = ({ content }) => {
      ...
      <div className="p-6">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div className="ml-auto">
            {isPlaying && (<div className="flex items-center space-x-2 px-3 py-1 bg-green-100 dark:bg-green-900/30 rounded-full">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-6">
          <Button onClick={handlePlayPause} disabled={!content.trim()} className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-medium disabled:opacity-50">
            {isPlaying ? (<>
          ...
          <Button onClick={handleStop} disabled={!isPlaying} variant="outline" className="disabled:opacity-50">
            <Square className="h-4 w-4 mr-2"/>
            Stop
          </Button>

          <Button variant="outline" className="border-purple-300 text-purple-600 hover:bg-purple-50 dark:border-purple-700 dark:text-purple-400">
        ...
        <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-cyan-900/20 dark:to-blue-900/20 border border-cyan-200 dark:border-cyan-800">
          <p className="text-sm text-slate-600 dark:text-slate-400 text-center">
            {isPlaying ? (<span className="text-cyan-600 dark:text-cyan-400 font-medium">🎵 Reading your content out loud...</span>) : content.trim() ? ("✨ Ready to read! Click play to hear your content.") : ("📝 Add some content above to enable voice reading.")}
          </p>
        </div>
      </div>
    </Card>);
