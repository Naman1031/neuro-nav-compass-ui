...
export const ModernLearningHub = ({ content, setContent, isFocusMode, isReducedMotion }) => {
    ...
    return (<div className="space-y-8">
      ...
      <Card className={`backdrop-blur-xl bg-white/30 dark:bg-slate-800/30 border-white/20 dark:border-slate-700/30 shadow-2xl ${!isReducedMotion ? 'hover:shadow-3xl transition-all duration-500' : ''}`}>
        <div className="p-8">
          ...
          {/* Text Area */}
          <Textarea placeholder="Or paste your text here... I'll help make it perfect for your brain! 🧠✨" value={content} onChange={(e) => setContent(e.target.value)} className={`min-h-[200px] text-lg leading-relaxed resize-none border-0 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-2xl focus:ring-2 focus:ring-purple-500 ${isFocusMode ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''}`}/>

          {/* Action Button */}
          <div className="flex justify-center mt-6">
            <Button onClick={handleProcessContent} disabled={isProcessing || !content.trim()} className={`px-8 py-4 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold text-lg rounded-2xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${!isReducedMotion ? 'hover:scale-105 transition-all duration-300' : ''}`}>
              {isProcessing ? (<>
      ...
      {results && (<div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className={`backdrop-blur-xl bg-gradient-to-br from-yellow-100 to-orange-100 dark:from-yellow-900/20 dark:to-orange-900/20 border-yellow-200 dark:border-yellow-800 shadow-xl ${!isReducedMotion ? 'hover:shadow-2xl transition-all duration-500' : ''}`}>
            <div className="p-6">
              ...
              <div className="space-y-3">
                {results.concepts.map((concept, index) => (<div key={index} className="p-3 bg-white/60 dark:bg-slate-800/60 rounded-xl flex items-center">
          ...
          <Card className={`backdrop-blur-xl bg-gradient-to-br from-indigo-100 to-cyan-100 dark:from-indigo-900/20 dark:to-cyan-900/20 border-indigo-200 dark:border-indigo-800 shadow-xl ${!isReducedMotion ? 'hover:shadow-2xl transition-all duration-500' : ''}`}>
            <div className="p-6">
              ...
              <div className="space-y-3">
                {results.quiz.map((question, index) => (<div key={index} className="p-3 bg-white/60 dark:bg-slate-800/60 rounded-xl">
