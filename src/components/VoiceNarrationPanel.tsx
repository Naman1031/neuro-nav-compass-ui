...
export const VoiceNarrationPanel = ({ content }: VoiceNarrationPanelProps) => {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg">
          <p className="text-sm text-slate-600 dark:text-slate-400">
            {isPlaying ? (
              <span className="text-green-600 dark:text-green-400">🔊 Playing narration...</span>
            ) : content.trim() ? (
              "Ready to play. Click the play button to start narration."
            ) : (
              "Add some content to enable voice narration."
            )}
          </p>
        </div>
      </div>
