...
export const LearningDashboard = ({ content, setContent, isFocusMode, isReducedMotion }: LearningDashboardProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={`p-6 ${!isReducedMotion ? 'gentle-hover' : ''} border-calm-200 dark:border-slate-700 shadow-lg`}>
        <div className="space-y-4">
          ...
          <Textarea
            ...
            className="min-h-[200px] text-base leading-relaxed resize-none focus:ring-2 focus:ring-calm-500 border-calm-200 dark:border-slate-600"
          />
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-slate-50 dark:bg-slate-800 p-4 rounded-xl">
            ...
            <Button
              ...
              className="btn-calm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isProcessing ? (
                <>
      ...
      {/* Results Section */}
      {(simplifiedText || summary || definitions.length > 0 || quizQuestions.length > 0) && (
        <div className="grid md:grid-cols-2 gap-6">
          {/* Simplified Text */}
          {simplifiedText && (
            <Card className={`p-6 ${!isReducedMotion ? 'gentle-hover' : ''} border-sage-200 dark:border-slate-700 shadow-lg`}>
              <div className="flex items-center space-x-2 mb-4">
            ...
            <Card className={`p-6 ${!isReducedMotion ? 'gentle-hover' : ''} border-sage-200 dark:border-slate-700 shadow-lg`}>
              ...
              <div className="space-y-2">
                {definitions.map((def, index) => (
                  <div key={index} className="p-3 bg-sage-50 dark:bg-slate-800 rounded-lg">
